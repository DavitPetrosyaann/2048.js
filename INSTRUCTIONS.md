...
├── components/
│   ├── Board.tsx                   # Renders the 2048 grid, cells, and gesture handlers
│   └── Tile.tsx                    # Renders individual tiles with transition animations
├── utils/
│   └── game.ts                     # Board setup, slide movements, merging, and score tracking logic
├── App.tsx                         # Main container layout, sidebars, stats, and theme toggling
...
├── tsconfig.json                   # TypeScript compiler configuration (targets "dist")
├── types.ts                        # Shared type definitions (Tile, Board, Direction, etc.)
...
If a game is hosted under a domain subpath (such as `prezento.am/games/2048/`), absolute references like `/assets/...` will fail with a 404 error because they point to the root domain. 
...
export default defineConfig(({ mode }) => {
  return {
    base: '/games/2048/', // Change to the correct subfolder path (with leading and trailing slashes)
    // ...other config
  };
});
```
